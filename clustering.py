...
def get_distance_between_points(r1, r2, t1, t2, theta_all):
    """
        This function calculates the distance between two points according to theta_all.
...
def clustering_lines(image, lines, theta_all):
    ...
    if len(rho_theta) < 7 and lines[0] not in rho_theta and not check_dist_small(lines, rho_theta, theta_all, 0):
        rho_theta.append(lines[0])
    elif (len(rho_theta) < 7 and lines[len(lines) - 1] not in rho_theta and not check_dist_small(lines, rho_theta,
                                                                                                 theta_all, 1)):
        rho_theta.append(lines[len(lines) - 1])
    elif (len(rho_theta) == 7 and (
            (lines[len(lines) - 1] not in rho_theta) and not check_dist_small(lines, rho_theta, theta_all, 1))):
        rho_theta = remove_min_dist_line(rho_theta, theta_all)
        rho_theta.append(lines[len(lines) - 1])
    elif (len(rho_theta) == 7 and (
            (lines[0] not in rho_theta) and not check_dist_small(lines, rho_theta, theta_all, 0))):
        rho_theta = remove_min_dist_line(rho_theta, theta_all)
        rho_theta.append(lines[0])
