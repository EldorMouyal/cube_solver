...
def get_hough_params_by_base_threshold(image, theta, base_threshold):
    ...
    while base_threshold > 0:
        ...
        if (bins_status == 1):  #Too many lines
            ...
            flagForMany = True
        elif (bins_status == -1):  #Too littile lined
...
def create_thetas_from_iterations(lines, theta):
    ...
    """
    thetas = []  # List of theta values of lines
    for i in range(0, 400, 20):  # Dividing to 20 bins
...
def filter_lines_unusual_thetas(lines, theta):
    """
        This function filters the lines which their theta is not between thatas on side bins.
    ...
    min_theta_possible = find_thetas_possible(theta, thetas, min_bin_index_with_lines, max_bin_index_with_lines)[
        0]  # minimum theta of all lines
    max_theta_possible = find_thetas_possible(theta, thetas, min_bin_index_with_lines, max_bin_index_with_lines)[
        1]  # maximum theta of all lines
    lines_filtered = []
    for line in lines:
        _rho, _theta = line[0]
        if (
                max_theta_possible >= _theta >= min_theta_possible):  # if theta is between minimum and maximum theta
