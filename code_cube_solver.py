...
def check_places(places):
    ...
    for place in places:
        if(place<=100):
            places_spreading_left=True
        if(place>=300):
            places_spreading_right=True
    if (places_spreading_left==True and places_spreading_right==True):
        return True
    elif (places_spreading_left==False and places_spreading_right==False):
        return True
    else:
        return False
...
def find_hough_params(edges,min_angle,max_angle):
    rhos = []
    thetas = []
    base_threshold=170
    pass_thetas=True
